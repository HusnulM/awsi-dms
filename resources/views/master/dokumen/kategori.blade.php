...
<div class="modal fade" id="modal-add-department">
    <form action="{{ url('master/kategoridok/save') }}" method="post">
        @csrf
...
<div class="modal fade" id="modal-edit-department">
    <form action="{{ url('master/kategoridok/update') }}" method="post">
        @csrf
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
                            ...
                            <tbody id="tbl-edit-dept-body">
                                <tr>
                                    <td>
                                        ...
                                        <input type="hidden" class="form-control" name="ktid" id="ktid">
                                    </td>
                                </tr>
                            </tbody>
                        </table>  
...
<script>
    $(document).ready(function(){
        $("#tbl-dept-master").DataTable({
            serverSide: true,
            ajax: {
                url: base_url+'/master/kategoridok/list',
                data: function (data) {
                    data.params = {
                        sac: "sac"
                    }
        ...
        $('#tbl-dept-master tbody').on( 'click', '.button-delete', function () {
            ...
            window.location = base_url+"/master/kategoridok/delete/"+selected_data.id;
        });
        $('#tbl-dept-master tbody').on( 'click', '.button-edit', function () {
            ...
            $('#kategori').val(selected_data.kategori);
            $('#ktid').val(selected_data.id);
